...
// ── Import parser ─────────────────────────────────────────────────────────────
function parseBookingFile(file, targetUnit) {
    ...
    reader.onload = (e) => {
      try {
        ...
        for (const row of rows) {
          // Booking.com column names (German)
          const guest = row["Gästename(n) "] || row["Gästename(n)"] || row["Guest name"] || row["Gast"] || "";
          const checkIn = toISO(row["Anreise"] || row["Check-in"] || row["Arrival"]);
          const checkOut = toISO(row["Abreise"] || row["Check-out"] || row["Departure"]);
          const persons = parseInt(row["Personen"] || row["Guests"] || 1) || 1;
          const price = parsePrice(row["Preis"] || row["Price"] || row["Total"] || 0);
          const nights = parseInt(row["Aufenthaltsdauer (Nächte)"] || row["Nights"] || getNights(checkIn, checkOut)) || getNights(checkIn, checkOut);
          const phone = String(row["Telefonnummer"] || row["Phone"] || "");
          const notes = String(row["Bemerkungen"] || row["Remarks"] || row["Notes"] || "");
          const source = row["Gebucht von"] ? "Booking.com" : (row["Portal"] || "Booking.com");
          const statusRaw = String(row["Status"] || "ok").toLowerCase();
          const status = statusRaw === "ok" || statusRaw === "confirmed" ? "confirmed" :
                         statusRaw === "cancelled" || statusRaw === "storniert" ? "checkedout" : "confirmed";
...
export default function BookingDashboard() {
  ...
  async function loadBookings() { try { setBookings(await fetchBookings()); } catch {} }
  async function loadUnits() { try { setUnits(await fetchUnits()); } catch {} }
  ...
  function openEdit(b) { setFormData({...b}); setFormError(""); setDeleteConfirm(false); setFormMode("edit"); setSelected(null); }
  ...
  async function handleSave() {
    ...
    const payload = {
      guest: formData.guest.trim(), unit: formData.unit,
      check_in: formData.check_in, check_out: formData.check_out,
      nights, persons: Number(formData.persons), status: formData.status,
      price: Number(formData.price), phone: formData.phone || null,
      source: formData.source, notes: formData.notes || null,
    };
    try {
      if (formMode === "add") {
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <div style={{display:"flex", gap:8, alignItems:"center"}}>
          <button style={S.iconBtn} onClick={() => setImportMode(true)} title="Buchungen importieren">📥</button>
        ...
        <div style={S.list}>
          {filtered.length === 0
            ? <div style={S.empty}>
                ...
                <div style={{fontSize:12, color:"#4B5563"}}>Tippe auf „+ Buchung" oder „📥" zum Importieren.</div>
              </div>
      ...
      {importMode && (
        <BottomSheet onClose={() => setImportMode(false)} tall>
          <ImportSheet
            units={unitNames}
            existingBookings={bookings}
            onImport={async (newBookings) => {
              setSaving(true);
              try {
                const inserted = [];
                ...
                setBookings(prev => [...prev, ...inserted].sort((a,b) => a.check_in.localeCompare(b.check_in)));
                setImportMode(false);
...
// ── ImportSheet ───────────────────────────────────────────────────────────────
function ImportSheet({ units, existingBookings, onImport, saving, onClose }) {
  ...
  async function handleFile(e) {
    ...
    setError("");
    try {
      const parsed = await parseBookingFile(file, selectedUnit);
      if (parsed.length === 0) { setError("Keine Buchungen gefunden. Bitte prüfe die Datei."); return; }
      // Filter duplicates: skip if same guest + check_in already exists
      const filtered = parsed.filter(nb =>
        !existingBookings.some(eb => eb.guest === nb.guest && eb.check_in === nb.check_in && eb.unit === nb.unit)
      );
      setPreview({ all: parsed, new: filtered });
    } catch (e) {
  ...
  return (
    <>
      <div style={S.modalHeader}>
        <div style={S.modalGuest}>📥 Buchungen importieren</div>
        <button style={S.closeBtn} onClick={onClose}>✕</button>
      </div>
      <div style={S.formScroll}>
        <div style={S.importInfo}>
          Lade eine Excel- oder CSV-Datei von <strong>Booking.com</strong> hoch.<br/>
          Booking → Buchungen → „CSV herunterladen"
        </div>

        <Field label="UNTERKUNFT">
          <select style={S.input} value={selectedUnit} onChange={e => { setSelectedUnit(e.target.value); setPreview(null); }}>
            {units.map(u => <option key={u} value={u}>{u}</option>)}
          </select>
        </Field>

        <button style={{...S.uploadBtn}} onClick={() => fileRef.current.click()}>
          📂 Datei auswählen (.xls, .xlsx, .csv)
        </button>
        ...
        {preview && (
          <div style={S.previewBox}>
            <div style={{...S.dateLabel, marginBottom:8}}>VORSCHAU</div>
            <div style={S.previewStats}>
              <div style={S.previewStat}><span style={{fontSize:20}}>{preview.all.length}</span><span>gefunden</span></div>
              <div style={S.previewStat}><span style={{fontSize:20, color:"#4ADE80"}}>{preview.new.length}</span><span>neu</span></div>
              <div style={S.previewStat}><span style={{fontSize:20, color:"#FBBF24"}}>{preview.all.length - preview.new.length}</span><span>bereits vorhanden</span></div>
            </div>
            {preview.new.slice(0,3).map((b,i) => (
              <div key={i} style={S.previewRow}>
                <div style={{fontSize:13, fontWeight:600, color:"#F9FAFB"}}>{b.guest}</div>
                <div style={{fontSize:11, color:"#6B7280"}}>{formatDate(b.check_in)} → {formatDate(b.check_out)} · €{b.price}</div>
              </div>
            ))}
            {preview.new.length > 3 && <div style={{fontSize:11, color:"#4B5563", marginTop:6}}>… und {preview.new.length-3} weitere</div>}
          </div>
        )}

        {preview && preview.new.length > 0 && (
          <button
            style={{...S.actionBtn, ...S.actionBtnPrimary, width:"100%", marginTop:16, opacity:saving?0.7:1}}
            onClick={() => onImport(preview.new)}
            disabled={saving}
          >
            {saving ? "Importiere…" : `✅ ${preview.new.length} Buchungen importieren`}
          </button>
        )}
        {preview && preview.new.length === 0 && (
          <div style={{...S.formError, background:"rgba(251,191,36,0.1)", color:"#FBBF24", borderColor:"rgba(251,191,36,0.2)", marginTop:12}}>
            Alle Buchungen sind bereits vorhanden.
          </div>
        )}
      </div>
    </>
...
const S = {
  ...
  importInfo:{background:"#0F1117",borderRadius:10,padding:"12px 14px",marginBottom:16,fontSize:13,color:"#9CA3AF",lineHeight:1.6},
  uploadBtn:{width:"100%",padding:"14px",borderRadius:12,border:"2px dashed #374151",background:"transparent",color:"#9CA3AF",fontSize:14,fontWeight:500,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginBottom:16,textAlign:"center"},
  previewBox:{background:"#0F1117",borderRadius:12,padding:"14px",marginTop:12},
  previewStats:{display:"flex",gap:12,marginBottom:12},
  previewStat:{flex:1,background:"#161B27",borderRadius:8,padding:"8px",textAlign:"center",display:"flex",flexDirection:"column",gap:2,fontSize:11,color:"#6B7280"},
  previewRow:{padding:"8px 0",borderBottom:"1px solid #1F2937"},
};
